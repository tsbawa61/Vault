...
// Firebase Operational Vault Configuration Object Context
...
// Platform System Runtime Global Context States
let activeSessionUser = null;
let realtimePacksUnsubscribe = null;

// Structural UI Lifecycle Initialization Hook
...
function initViewRouterLinks() {
    ...
    document.getElementById("txt-login-role").addEventListener("change", (e) => {
        document.getElementById("grp-login-pass").style.display = (e.target.value === "SUPER_USER") ? "none" : "block";
    });
    ...
    // Core Administrative Entity Lifecycle Submission Form Pipeline Mappings
...
// Security Profile Verification & Operational Window Constraint Checks
async function processSecureProfileAuthentication() {
    const selectedRole = document.getElementById("txt-login-role").value;
    const emailInput = document.getElementById("txt-login-email").value.trim();
    const passwordInput = document.getElementById("txt-login-pass").value;

    if (!emailInput) return alert("Email identity must be defined.");

    try {
        if (selectedRole === "SUPER_USER") {
            // Super User Bypass Profile 
            activeSessionUser = { userNo: "0000", role: "SUPER_USER", name: "Developer Super Admin", ownerUserNo: "000" };
        ...
        // Boundary constraints validation parsing
        ...
        if (!userDoc.active) return alert("Account flag is current inactive or archived.");

        activeSessionUser = userDoc;
        renderAuthorizedWorkspaceSession();
    } catch (crash) {
        reportRuntimeCrash(activeSessionUser?.name || "Anonymous", activeSessionUser, crash);
    }
}
...
function renderAuthorizedWorkspaceSession() {
    ...
    // Grant contextual interface modification controls matching permission flags
    if (activeSessionUser.role === "SUPER_USER" || activeSessionUser.role === "OWNER") {
        ...
        // Read-only mapping defaults constraints enforcement logic
    ...
    // Initialize standard multi-tenant snapshot filters and populate choice matrices
...
function performSessionLogoutAction() {
    ...
    document.querySelectorAll("#nav-container .nav-link").forEach(link => {
        if(link.id !== "nav-home" && link.id !== "nav-login") link.classList.add("d-none");
    });
    document.getElementById("nav-login").classList.remove("d-none");
    showActiveFrame("sec-home");
}

// Code Offset Auto-Population Processing Core Engine
...
async function executeDynamicAutopopulateMenuTask() {
    ...
    try {
        ...
        await batch.commit();
        alert("Autopopulate routing executed successfully. Catalog elements appended safely.");
        refreshAllAdministrativeTables();
    } catch(err) {
...
// Live Real-Time Multi-Tenant Query Tracking Pipeline
    ...
    realtimePacksUnsubscribe = onSnapshot(qPacks, async (snapshot) => {
        try {
            ...
            if(urgentExpiries.length === 0) {
                tExp.innerHTML = `<tr><td colspan="4" class="text-center text-muted py-2">No pending package expirations tracked inside the 10-day timeline.</td></tr>`;
            } else {
...
// Form ADM Implementations (Sequential Code Generation Engine Blocks)
async function processCategoryADMFormSubmission(e) {
    ...
    try {
        ...
        const docId = `${activeSessionUser.ownerUserNo}_CAT_${nextCode}`;
        
        await setDoc(doc(db, "serviceCategories", docId), {
...
async function processServiceADMFormSubmission(e) {
    ...
    try {
        ...
        const docId = `${activeSessionUser.ownerUserNo}_SRV_${nextCode}`;
...
async function processSubServiceADMFormSubmission(e) {
    ...
    try {
        ...
        const docId = `${activeSessionUser.ownerUserNo}_SUB_${nextCode}`;

        await setDoc(doc(db, "subServices", docId), {
...
async function processUserADMFormSubmission(e) {
    ...
    try {
        ...
        const docId = `${activeSessionUser.ownerUserNo}_USR_${runningSerial}`;
...
// Interactive Table Renderers & Dynamic Choice Population Filters
async function refreshAllAdministrativeTables() {
    ...
    // Render Service Categories Data Grid Row Nodes
    renderTable("serviceCategories", "tbl-adm-categories", (data, id) => {
        const tr = document.createElement("tr");
        tr.innerHTML = `<td>${data.catCode}</td><td>${data.catName}</td><td>${data.catDescription}</td><td>${data.active}</td><td><button class="btn btn-danger btn-xs py-0" onclick="globalDeleteNode('${colName}','${id}')">Delete</button></td>`;
        return tr;
    });

    // Render Sub-Services Data Grid Row Nodes (Displaying mapped field parent links)
    renderTable("subServices", "tbl-adm-subservices", (data, id) => {
        const tr = document.createElement("tr");
        tr.innerHTML = `<td>${data.subServiceCode}</td><td>Service: ${data.serviceCode}</td><td>${data.subServiceName}</td><td>${data.rate}</td><td>${data.durationMinutes}</td><td>${data.active}</td><td>-</td>`;
        return tr;
    });

    // Render Master Templates Package Row Configurations
    renderTable("commonServicePacks", "tbl-adm-commonpacks", (data, id) => {
        const tr = document.createElement("tr");
    ...
    // Render User Management Row Nodes
    renderTable("users", "tbl-adm-users", (data, id) => {
        const tr = document.createElement("tr");
        tr.innerHTML = `<td>${data.userNo}</td><td>${data.role}</td><td>${data.name}</td><td>${data.email}</td><td>${data.active}</td><td>-</td>`;
        return tr;
    });
}
