...
async function processSecureProfileAuthentication() {
    ...
    if (!emailInput) return alert("Please enter your sign-in email address.");

    try {
        if (selectedRole === "SUPER_USER") {
            if (emailInput !== 'bawa.codes@gmail.com') { // Bound to your superuser setup
                alert("Sign In Blocked: This email address is not registered as a System Administrator.");
                return;
            }
            ...
            if (!otpInputVal) {
                ...
                alert("A secure sign-in token has been sent to the administrator registration inbox.");
                return;
            }

            if (parseInt(otpInputVal, 10) !== window.tempSessionOtpStorage) {
                alert("Incorrect Token: The 6-digit confirmation code does not match.");
                return;
            }
        ...
        if (!passwordInput) return alert("Please enter your password secret phrase.");
        ...
        if (res.empty) return alert("Sign In Failed: No user profile matching those details was found.");
        ...
        if (userDoc.startDate && isoToday < userDoc.startDate) return alert("Access Status Alert: Your profile schedule settings are pre-operational.");
        if (userDoc.expiryDate && isoToday > userDoc.expiryDate) return alert("Access Status Alert: Salon management contract package has expired.");
        if (!userDoc.active) return alert("Access Status Alert: This account profile is currently marked inactive.");
        ...
        await handleTelemetryAlert("Security Profile Sign In Node", crash);
    }
}
